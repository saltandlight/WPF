...
| x:ClassModifier  | 루트 엘리먼트의 어트리뷰트, x:Class와 함께 사용되어야 함     | x:Class로 정의된 `클래스`의 접근제한자를 정의함. 기본값은 `public`임. 어트리뷰트값으로는 반드시 C#에서 사용되는 public이나 internal 같은 단어만 가능함. |
| x:Code           | XAML 내부의 어디서나 엘리먼트에 사용. x:Class와 함께 사용되어야 함 | x:Class로 정의된 클래스 안에서 프로그래밍 코드를 넣기 위해 사용됨 |
| x:FieldModifiler | 루트를 제외한 엘리먼트에 사용되는 어트리뷰트. x:Name이나 그와 동등한 마크업 확장식과 같이 사용되어야 함 | 엘리먼트를 위해 만들어질 `멤버 필드들`을 위한 접근제한자 설정함. 기본 값은 `internal`임. x:ClassModifier 처럼 C#에서 사용되는 public이나 private 같은 단어만 가능 |
| x:Key            | IDictionary를 구현한 부모 엘리먼트를 가진 자식 엘리먼트의 어트리뷰트로 사용됨 | 부모 딕셔너리에 아이템을 추가할 때, 추가되는 아이템의 키 값을 정의함 |
| x:Name           | 루트 엘리먼트를 제외한 엘리먼트에 어트리뷰트로 사용. x:Class와 함께 사용되어야 함 | 엘리먼트에 생성될 필드의 이름을 선택. 프로그래밍 코드에서 참조될 수 있도록 함. |
| x:Shared         | ResourceDictionary의 어떤 엘리먼트이던지 어트리뷰트로 사용 가능. 단지 XAML이 컴파일된 경우에만 동작함 | 여러 지역의 동일한 리소스 인스턴스의 공유 막고자 false로 설정 가능함. |
| x:Subclass       | 루트 엘리먼트에만 사용 가능한 엘리먼트. x:Class와 함께 사용되어야 함 | x:Class로 정의된 클래스의 하위 클래스를 정의. XAML의 컨텐트 프로퍼티에 내용을 간직하고 있음. partial 클래스의 지원이 되지 않는 언어에서는 전체 닷넷 네임 스페이스명을 위해 사용할 수도 있음. |
| x:TypeArguments  | 루트 엘리먼트에만 사용 가능한 엘리먼트. x:Class와 함께 사용되어야 함 | 특정 타입만 사용 가능하도록 루트 엘리먼트를 제너릭 클래스로 만듬 예를 들면 `<Int32>` 나` List<String>`을 인수로 사용하는 `List<T>`클래스를 만듬 |
| x:Uid            | 어떤 엘리먼트에든 사용 가능한 엘리먼트                       |                                                              |
| x:XData          | IXmlSerializable 타입의 프로퍼티를 위한 값으로 사용되는 엘리먼트 | XAML 파서에게 불분명하게 남겨진 어떤 임의의 XML 데이터 일부분. |

- XAML 언어 네임스페이스의 키워드들과 혼동될 수 있는 마크업 확장식을 정리함
- System.Windows.MArkup 네임스페이스에 있는 닷넷 클래스들
- 각 클래스의 Extension 접미사는 일반적으로 사용 안 함 

**XAML 언어 네임 스페이스의 마크업 확장식**

| 마크업 확장식 | 설명                                                         |
| ------------- | ------------------------------------------------------------ |
| x:Array       | 닷넷의 배열을 의미함. X:Array 엘리먼트의 자식들은 배열의 엘리먼트들. 배열의 타입을 정의하기 위해서는 x:Type을 이용함 |
| x:Null        | 널 값 참조를 의미함                                          |
| x:Static      | 프로그래밍 코드에 정의된 스태틱 속성의 프로퍼티, 필드, 상수, 열거형을 가리킴. XAML이 컴파일되었을 때, 동일한 어셈블리에 있는 퍼블릭 속성이 아닌 멤버들일 수도 있음. Member 를 사용해야 하는 경우, 해당 타입이 기본 네임스페이스에 속하지 않으면 XML 네임스페이스 접두사를 가지고 있어야 함. |
| x:Type        | C#에서 typeof 키워드와 같은 System.Type 클래스의 인스턴스를 의미함. TypeName을 사용해야 하는 경우에 정의되는 해당 타입이 기본 네임스페이스에 속하지 않으면 XML 네임스페이스 접두사를 갖고 있어야 함. |

## 결론

- 내부적인 처리과정을 이해할 수 없으면, 직접 프로그래밍 코드에서 타입 컨버터를 호출하는 것은 지양...!
- 타입 컨버터에 대응하는 많은 클래스들은 타입 컨버터와 동일한 처리를 하는 Parse 스태틱메소드를 가짐 
- XAML에서 특별한 방식으로 다루는 널 값 같은 단순한 개념들은 마크업 확장식을 사용해서 표현할 수 있다.
- 이런 기능적인 특징들이 XAML을 사용하기 편하게 만들어줌
- WPF는 XAML의 사용에 아주 최적화되어 있음 -> 코딩 중에 불편한 점이 있음.
  - WPF는 풍부한 표현을 가능하게 하는 많은 작은 빌딩 블록들을 노출
  - WPF 프로그램은 윈폼 프로그램보다 더 많은 객체를 수동으로 만들어야 함 
  - XAML은 복잡한 객체 간 계층구조를 간결하게 표현하는 데 중점을 둠 -> 타입 컨버터처럼 XAML 내에서 사용 가능한 새로운 문법을 만듦

**XAML이 XML 기반이라는 이유로 오해하는 부분-> 논쟁거리**

### 논쟁1. XML은 데이터 타입을 너무 장황하게 표현한다.

- XML로 표현하는 것을 좋아하는 사람은 없음 그러나... 그런 일을 대신해주는 프로그램들이 있음 
- 적절한 XSD(XML Schema Definition)를 이용 -> 자동 완성 기능 사용 또는 엘리먼트(`<>`) 표시조차도 타이핑할 필요가 없음
- 결론: 자동 완성 기능을 사용하는 것은 좋지만 직접 작성하면서 XAML에 익숙해지는 것이 좋다.

### 논쟁2. XML 기반 시스템은 성능이 형편없다.

- XML은 데이터의 효과적인 표현보다는 다른 플랫폼이나 다른 기기 간의 상호운용성에 더 중점을 둠
- 상대적으로 파싱하기에 느리고 많은 데이터를 처리해야 하는 것은 사실임...
- 왜 WPF 프로그램들이 그 책임을 져야 하는가?
  - 일반적인 WPF 프로그램을 사용할 때는, XAML 을 BAML로 컴파일 -> 런타임 시에 파싱해야 하는 크기를 줄이고 성능을 높임
  - BAML은 XAML보다 용량 면에서는 더 작아지고 런타임 시에는 효율적으로 사용되도록 최적화되어 있음
  - XML에서 제기되는 성능의 함정: 기술 자체의 부족함보다는 XML의 이점을 충분휘 발휘할 수 있도록 하기 위한 충분한 개발 시간에 달려 있음.